...
struct SnowCanvasWatermarkConfig {
    ...
    double gap = 56.0;
    double opacity = 0.16;
    // Corner timestamp overlay ("watermark camera" style badge). Rendered on
    // the Qt side only; position: 0 = bottom-left, 1 = bottom-right.
    bool cornerTimeEnabled = false;
    int cornerTimePosition = 0;
};
...
inline bool operator==(const SnowCanvasWatermarkConfig& lhs, const SnowCanvasWatermarkConfig& rhs) {
    return lhs.color == rhs.color && lhs.text == rhs.text &&
           ...
           snowCanvasExactDoubleEqual(lhs.opacity, rhs.opacity) &&
           lhs.cornerTimeEnabled == rhs.cornerTimeEnabled &&
           lhs.cornerTimePosition == rhs.cornerTimePosition;
}
...
// Qt-side corner timestamp overlay state ("watermark camera" style badge).
// These process-global anchors carry the corner-time options through the Qt
// rendering layer without touching the engine FFI state; the C FFI config
// struct and the Rust engine are intentionally left unchanged. The value
// survives across screenshots within one process and resets on restart.
inline bool& snowCanvasWatermarkCornerTimeEnabled() {
    static bool value = false;
    return value;
}

inline int& snowCanvasWatermarkCornerTimePosition() {
    static int value = 0;
    return value;
}
