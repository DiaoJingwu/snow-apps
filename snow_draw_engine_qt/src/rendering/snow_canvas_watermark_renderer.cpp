...
void WatermarkPatternRenderer::render(QPainter& painter, const WatermarkRenderRequest& request) {
    ...
    QString text = QString::fromUtf8(configuration.watermark_text.data(), textLength)
                       .normalized(QString::NormalizationForm_C);
    if (text.contains(QLatin1Char('{'))) {
        const QDateTime now = QDateTime::currentDateTime();
        text.replace(QLatin1String("{datetime}"),
                     now.toString(QStringLiteral("yyyy-MM-dd HH:mm")));
        text.replace(QLatin1String("{time}"),
                     now.toString(QStringLiteral("HH:mm")));
        text.replace(QLatin1String("{date}"),
...
void renderCornerTimestamp(QPainter& painter, const QRectF& surfaceBounds,
                           const QRectF& renderArea, int cornerPosition) {
    if (surfaceBounds.width() <= 0.0 || surfaceBounds.height() <= 0.0) {
        return;
    }
    const QDateTime now = QDateTime::currentDateTime();
    const QString timeText = now.time().toString(QStringLiteral("HH:mm"));
    const QString dateText = now.date().toString(QStringLiteral("yyyy.MM.dd"));
    const QString weekText =
        QLocale(QLocale::Chinese, QLocale::China).toString(now.date(), QStringLiteral("dddd"));

    const qreal surfaceHeight = surfaceBounds.height();
    const qreal margin = std::clamp(surfaceHeight * 0.030, 14.0, 44.0);
    const qreal timePixelSize = std::clamp(surfaceHeight * 0.070, 24.0, 96.0);
    const qreal datePixelSize = std::clamp(timePixelSize * 0.34, 12.0, 30.0);

    QFont timeFont = painter.font();
    timeFont.setPixelSize(std::max(1, qRound(timePixelSize)));
    timeFont.setBold(true);
    QFont dateFont = timeFont;
    dateFont.setPixelSize(std::max(1, qRound(datePixelSize)));
    dateFont.setBold(false);

    const QFontMetricsF timeMetrics(timeFont);
    const QFontMetricsF dateMetrics(dateFont);
    const qreal timeWidth = timeMetrics.horizontalAdvance(timeText);
    const qreal timeHeight = timeMetrics.height();
    const qreal dateWidth = dateMetrics.horizontalAdvance(dateText);
    const qreal weekWidth = dateMetrics.horizontalAdvance(weekText);
    const qreal dateHeight = dateMetrics.height();
    if (timeWidth <= 0.0 || dateWidth <= 0.0) {
        return;
    }

    const qreal innerGap = timePixelSize * 0.26;
    const qreal dividerWidth = std::max<qreal>(2.0, timePixelSize * 0.06);
    const qreal dateRowGap = dateHeight * 0.18;
    const qreal dateColumnHeight = dateHeight * 2.0 + dateRowGap;
    const qreal columnWidth = std::max(dateWidth, weekWidth);
    const qreal blockHeight = std::max(timeHeight, dateColumnHeight);
    const qreal blockWidth = timeWidth + innerGap + dividerWidth + innerGap + columnWidth;

    qreal cursorX = surfaceBounds.left() + margin;
    if (cornerPosition == 1) {
        cursorX = surfaceBounds.right() - margin - blockWidth;
    }
    const qreal blockTop = surfaceBounds.bottom() - margin - blockHeight;
    if (blockTop < surfaceBounds.top() || cursorX < surfaceBounds.left() ||
        cursorX + blockWidth > surfaceBounds.right() + 0.5) {
        return;
    }

    QRectF clipBounds = renderArea.normalized();
    if (clipBounds.isEmpty()) {
        clipBounds = surfaceBounds;
    } else {
        clipBounds = clipBounds.intersected(surfaceBounds);
        if (clipBounds.isEmpty()) {
            return;
        }
    }
    ...
    painter.save();
    painter.setClipRegion(QRegion(clipBounds.toAlignedRect()), Qt::IntersectClip);
    painter.setRenderHint(QPainter::TextAntialiasing, true);

    const QColor mainColor(255, 255, 255, 245);
    const QColor shadowColor(0, 0, 0, 150);
    const QColor dividerColor(255, 170, 40, 240);
    const qreal shadowOffset = std::max<qreal>(1.0, timePixelSize * 0.035);
    const qreal timeTop = blockTop + (blockHeight - timeHeight) / 2.0;
    const qreal dateColumnTop = blockTop + (blockHeight - dateColumnHeight) / 2.0;
    const qreal dividerX = cursorX + timeWidth + innerGap;
    const qreal columnX = dividerX + dividerWidth + innerGap;

    const auto drawShadowedText = [&](const QPointF& baseline, const QFont& font,
                                      const QString& text) {
    ...
    drawShadowedText(QPointF(cursorX, timeTop + timeMetrics.ascent()), timeFont, timeText);
    painter.fillRect(QRectF(dividerX, blockTop, dividerWidth, blockHeight), dividerColor);
    drawShadowedText(QPointF(columnX, dateColumnTop + dateMetrics.ascent()), dateFont, dateText);
    drawShadowedText(
        QPointF(columnX, dateColumnTop + dateHeight + dateRowGap + dateMetrics.ascent()), dateFont,
        weekText);
