...
SnowCanvasWatermarkConfig SnowCanvasWidget::Impl::canvasWatermarkConfig() const {
    ...
    // The corner timestamp options live in the Qt-side anchors, so re-attach
    // them whenever the engine-backed config is read back.
    config.cornerTimeEnabled = snowCanvasWatermarkCornerTimeEnabled();
    config.cornerTimePosition = snowCanvasWatermarkCornerTimePosition();
    return config;
}
...
bool SnowCanvasWidget::Impl::setCanvasWatermarkConfig(const SnowCanvasWatermarkConfig& config) {
    ...
    const bool cornerTimeOverlayChanged =
        snowCanvasWatermarkCornerTimeEnabled() != config.cornerTimeEnabled ||
        snowCanvasWatermarkCornerTimePosition() != config.cornerTimePosition;
    ...
    if (applied) {
        QRegion repaintRegion = previousPreviewRegion;
        // The corner timestamp badge is rendered by the Qt decoration layer, so
        // it needs a repaint even when the tiled watermark region is empty.
        if (config.cornerTimeEnabled || cornerTimeOverlayChanged) {
            repaintRegion += widget.rect();
        }
        if (!repaintRegion.isEmpty()) {
            widget.update(repaintRegion);
        }
    }
    return applied;
}
...
void SnowCanvasWidget::Impl::applyPendingWatermarkPreview() {
    ...
    const bool cornerTimeOverlayChanged =
        snowCanvasWatermarkCornerTimeEnabled() != pendingWatermarkPreview->cornerTimeEnabled ||
        snowCanvasWatermarkCornerTimePosition() != pendingWatermarkPreview->cornerTimePosition;
    const QRegion previousRegion = watermarkViewRenderRegion();
    watermarkPreview = *pendingWatermarkPreview;
    pendingWatermarkPreview.reset();
    snowCanvasWatermarkCornerTimeEnabled() = watermarkPreview->cornerTimeEnabled;
    snowCanvasWatermarkCornerTimePosition() = watermarkPreview->cornerTimePosition;
    updateWatermarkPresentationInfo();
    QRegion changedRegion = previousRegion.united(watermarkViewRenderRegion());
    // The corner timestamp badge is rendered by the Qt decoration layer, so
    // it needs a repaint even when the tiled watermark region is empty.
    if (watermarkPreview->cornerTimeEnabled || cornerTimeOverlayChanged) {
        changedRegion += widget.rect();
    }
