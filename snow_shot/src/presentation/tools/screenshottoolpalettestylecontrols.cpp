...
int watermarkCornerTimeModeValue(const SnowCanvasWatermarkConfig& config) {
    if (!config.cornerTimeEnabled) {
        return 0;
    }
    return config.cornerTimePosition == 0 ? 1 : 2;
}

QString watermarkCornerTimeModeText(int mode) {
    switch (((mode % 3) + 3) % 3) {
        case 1:
...
void ScreenshotToolPaletteStyleControls::addWatermarkControls(
    ...
    const int initialCornerTimeMode = watermarkCornerTimeModeValue(config);
    m_watermarkCornerTimeEditor = createScreenshotToolPaletteIconNumericValueButton(
        parent, "时间角标（点击或滚轮切换：关/左下/右下）", custom_outlined_icons::ToolWatermark(),
        initialCornerTimeMode, QStringLiteral("右下"), metrics);
    m_watermarkCornerTimeEditor->setObjectName(
        QStringLiteral("screenshotWatermarkCornerTimeEditor"));
    m_watermarkCornerTimeEditor->setValueText(watermarkCornerTimeModeText(initialCornerTimeMode));
    layout->addWidget(m_watermarkCornerTimeEditor);
    QObject::connect(m_watermarkCornerTimeEditor, &adqt::widgets::AdButton::clicked, receiver,
                     [this]() {
                         setWatermarkCornerTimeMode(
                             watermarkCornerTimeModeValue(m_watermarkConfig) + 1);
                     });
...
bool ScreenshotToolPaletteStyleControls::handleWatermarkWheel(const QPoint& globalPosition,
    ...
    if (m_watermarkCornerTimeEditor != nullptr && m_watermarkCornerTimeEditor->isVisible() &&
        m_watermarkCornerTimeEditor->rect().contains(
            m_watermarkCornerTimeEditor->mapFromGlobal(globalPosition))) {
        setWatermarkCornerTimeMode(watermarkCornerTimeModeValue(m_watermarkConfig) +
                                   (direction > 0 ? 1 : -1));
        return true;
    }
...
void ScreenshotToolPaletteStyleControls::setWatermarkCornerTimeMode(int mode) {
    const int normalizedMode = ((mode % 3) + 3) % 3;
    const bool enabled = normalizedMode != 0;
    const int position = normalizedMode == 2 ? 1 : 0;
    if (m_watermarkConfig.cornerTimeEnabled == enabled &&
        m_watermarkConfig.cornerTimePosition == position) {
        return;
    }
    m_watermarkConfig.cornerTimeEnabled = enabled;
    m_watermarkConfig.cornerTimePosition = position;
    updateWatermarkControls();
    notifyWatermarkConfigChanged();
}
...
void ScreenshotToolPaletteStyleControls::updateWatermarkControls() {
    ...
    if (m_watermarkCornerTimeEditor != nullptr) {
        const int cornerTimeMode = watermarkCornerTimeModeValue(m_watermarkConfig);
        m_watermarkCornerTimeEditor->setValue(cornerTimeMode);
        m_watermarkCornerTimeEditor->setValueText(watermarkCornerTimeModeText(cornerTimeMode));
    }
